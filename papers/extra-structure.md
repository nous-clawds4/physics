...
5. **A measure on $\mathrm{Obs}$**, if typicality of many $O$ is wanted instead of (or after) delayed forks from one $O$. Fermi-slice topology exists (Theorem 16) and does not count (Theorem 17). A measure is not written. Measurability of $R$ in the Fermi-slice product is extra toward a measure, not a measure (`papers/measurable-r.md`). Circularity of “count first, then typicality, then the measure that was used to count” is not resolved.
