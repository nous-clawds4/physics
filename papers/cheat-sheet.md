...
- **Measurability of $R$ (named).** Borel (etc.) graph in the Fermi-slice product is extra, toward a measure, not a measure. Not a grain. Not locality. `papers/measurable-r.md`
- **Closed-graph $R$ (named).** Closed graph (or closed out-stars) in the Fermi-slice topology is extra. Distinct from locality and from measurability. Not a grain. `papers/closed-graph-r.md`
- **Compact-valued stars (named).** Compact (or relatively compact) out-stars in the Fermi-slice topology are extra. Distinct from closed-graph, measurability, and locality. Compact is not finite (Theorem 17, `papers/neighborhood-uncountable.md`). Theorem 16: `papers/obs-as-a-space.md`. `papers/compact-stars.md`
