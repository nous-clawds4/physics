...
The unpublished 2005–07 texts already took an observer to be a point in an infinite-dimensional jet space. AFLB 2008 did not. That paper illustrated a counting problem inside the many-worlds interpretation. It did not state observer-space, and it did not equate the jet with a unique simply-connected analytic patch.

The novelty here, against those unpublished texts as well as against AFLB, is the unoccupied claim: the infinite jet is the unique simply-connected analytic patch; extra topologies are a relative ensemble, not a second elementary observer; evolutions split at the first $\tau$ the local patches disagree; there is no global four-manifold $W$ reconstructed from the observer; and there is no Born derivation.
...
The third claim is about motivation, and only about motivation. Many-worlds quantum theory has a well-known mismatch between two ways of saying "most." If worlds, or branches, or records, are counted equally, the Born rule is not what most observers see confirmed. If worlds are weighted by $|\psi|^2$, the Born rule is what most of the weight sees confirmed. Graham's remark, as the 2008 illustration quoted it, was that it is difficult to see what significance the Born measure can have when a simple count of the worlds contradicts it, given that those worlds were supposed to be on the same footing. A theory that talks about typical observers in the $|\psi|^2$ sense is already using as its measure of "most" the quantity whose status as probability was the question. That mismatch is why the present ontology does not take Hilbert-space worlds with $|\psi|^2$ measure as the space of observers. It is not a derivation of the Born rule. It is a reason to put the objects somewhere else.

The 2008 illustration (Strayhorn 2008) is that mismatch written as a number. It is an illustration, not this paper.

If the space of observers is Hilbert-space worlds, and the measure is $|\psi|^2$, then "the vast majority of observers see the Born rule confirmed" is true by the construction of the measure. It cannot be evidence for the measure. If the space of observers is something else, and the measure is counting, then what typical observers see is a later, empirical question about that space. This paper does not answer that question. It insists that the question not be begged by taking $|\psi|^2$ as the measure of observers.
...
It is not Stoica's background-freedom counting either (arXiv:2209.08623). Those objects are 3-slices in a quantum-gravity configuration space. The count still uses $|\Psi|$. This object is a 4-metric jet with a time direction.
...
It is not Hadley's program, in which closed timelike curves, or 4-geons, are asked to underwrite quantum logic or particle phenomenology (Hadley 1997). Hadley is nearby in one respect: a GR-first instinct, and a willingness to let classical geometry do work that is usually assigned to Hilbert space. Simply-connected is not a 4-geon. Hadley's 4-geons are typically not simply-connected. The present ontology does not take 4-geons as particles, does not take CTCs as particles, and does not rest non-uniqueness on that mechanism.
...
This paper does not derive the Born rule. It does not recover the Born rule as a coarse-grained approximation. It does not offer an action-principle derivation. Least action is named as the same kind of coarse-grained pattern as the Born rule, and is not derived here. It does not say that outcome counting gives rise to Born. Those sentences are prohibitions, not promises. A later paper might ask what typical observers in observer space see. That later paper would still not be a derivation of Born from Everett, because the objects would not be Everett worlds. This paper does not ask the question.
...
This paper does not make an experimental prediction. There is no number here. The 2008 illustration's spin-tree counts are not repeated, and were not a prediction of observer-space dynamics.
...
Stoica, O. C. Background freedom leads to many-worlds with local beables and probabilities. arXiv:2209.08623, 2022.
