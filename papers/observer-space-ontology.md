...
Physics uses the word *observer* constantly and almost never says what one is. This paper says what one is. An observer is a physical object: a point in a space of all possible observers. At an instant it is specified by a jet of the spacetime metric together with a future time direction. Its history is a curve in that space, parametrized by proper time.
...
At an instant, an observer is specified by two pieces of data.

The first is a jet of the spacetime metric at a point. A jet is the metric together with its derivatives at that point, to whatever order is needed to treat the observer as a local geometric object rather than a viewpoint. The 2008 papers said this in older language: the elementary observer includes "specification of the metric and power series," plus "a unit vector pointing in (defining) the forward time direction." The present paper takes that object to be a metric jet plus a time direction. It does not take the further 2008 step of reconstructing a whole four-manifold by analytic continuation from an infinite power series.

A jet is a small, ordinary idea with a technical name. The metric at a point is ten numbers, the local shape of spacetime where the observer is. The derivatives of the metric say how that shape is changing. Keep enough of those derivatives and you have a local physical object, not a coordinate choice and not a view from nowhere. How many derivatives are enough is a geometric question and is left open. The ontology needs only that the observer is that kind of object.

The second piece of data is that future time direction. Without it there is no later, and no proper time along which the object can move. With it, the observer is the sort of thing that has a history.

That is the whole of the instantaneous object. It is not a ray in Hilbert space. It is not a basis vector. It is not a four-velocity on a spacetime that has already been given (that last is a different use of the same English words; see Section 9). It is not a role. It is a physical object, small enough to be local, complete enough to know which way is later.
...
Observer space is the space of all such objects. A point of observer space is one observer at one instant: one metric jet, one future time direction.
...
An evolution is a curve in observer space, parametrized by proper time. Two evolutions are the same if and only if their metric jets and time directions agree at every proper time. They are distinct if they disagree at some $\tau$. That is a premise. It is not a candidate, and it is not reopened below.

The distinctness is not a fact about amplitudes. Hilbert-space weight does not make two evolutions the same, and it does not make them different. Grain is a separate question, and it stays open: the order of the jet, and what counts as a physically different disagreement at a given $\tau$, are for Geometry. The identity criterion does not wait on those answers. Same curve or not is already settled. How fine the jet is, and when a difference in the jet counts as physical, is not.
...
The identity criterion of §4 is a premise: same evolution if and only if jet and time direction agree at every proper time; distinct if they disagree at some $\tau$. Grain (jet order, what counts as physically different) is open. This paper does not settle the other open items flagged in §§2–4. The order of the jet is open. The dimension of observer space is open. Analyticity is open. Recovery of a 4-manifold is open. The mechanism of non-uniqueness is open, and is not identified with closed timelike curves. Matter fields are open. Those are not hidden assumptions. They are absences.
