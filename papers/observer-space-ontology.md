...
Physics uses the word *observer* constantly and almost never says what one is. This paper says what one is. An observer is a physical object: a point in a space of all possible observers. At an instant it is the infinite jet of a real-analytic metric at a point, together with a future time direction: equivalently, the unique simply-connected real-analytic patch determined by that jet. Its history is a curve in that space, parametrized by proper time.
...
The elementary observer is one object with two presentations.

The first presentation is the infinite jet of a real-analytic spacetime metric at a point, together with a future time direction. Infinite means every derivative. Analytic means the metric is real-analytic.

The second presentation is the unique simply-connected real-analytic patch determined by that jet. Unique analytic continuation. The patch is the jet written as a lump of spacetime.

These are not a choice. They are two ways of naming the same thing. Smooth is not enough. In the smooth category any jet is realized by some smooth metric, so the jet does not pick a unique lump. Analyticity is required. It is what makes the two presentations the same object.

The object is local. It is not the 2008 reconstruction of a whole four-manifold $W$ from an observer $O$. Analytic continuation here stops at the unique simply-connected patch. It does not assemble a global spacetime.

The picture is a lump hitchhiking a trajectory, the sort of object Echeverria, Klinkhammer, and Thorne used as a billiard ball (1991). That is the picture of the object, not the mechanism of the theory. The wormhole is not the ontology. The jet, equivalently the patch, is.

The time direction is the second piece of data. Without it there is no later, and no proper time along which the object can move. With it, the observer is the sort of thing that has a history.

One jet can sit in more than one global topology. The resulting patches agree to all orders at the point and are not the same lump. Those extra patches are the relative ensemble, not a second elementary observer. A macroscopic observer, a human, is the jet with the patch allowed to grow, the continuation run separately in each topology. The elementary object remains the jet/patch pair.

Gauge is settled as physics, not as a coordinate recipe. The object is that jet/patch pair, the lump. Two presentations of the same lump are the same observer. A diffeomorphism that moves the lump is a different observer. Two names for one lump are one observer.

That is the whole of the instantaneous elementary object. It is not a ray in Hilbert space. It is not a basis vector. It is not a four-velocity on a spacetime that has already been given (that last is a different use of the same English words; see Section 9). It is not a role. It is a physical object, local, analytic, complete enough to know which way is later.
...
Global topology labels do not get a vote. Extra patches that agree to all orders at the point, and differ only by the topology they sit in, are the relative ensemble, not a second elementary observer and not a split. A global solution-label does not get a vote. Amplitude still does not get a vote. Hilbert-space weight does not make two evolutions the same, and it does not make them different.
...
Distinct means physically distinct in the sense of §4: same evolution until the simply-connected analytic patches disagree; distinct at the first $\tau$ those patches disagree. That distinctness is not a Born-weight notion. One does not need $|\psi|^2$ to say that two evolutions are not the same. If one did need $|\psi|^2$ for that, counting distinct evolutions would be counting Born weight under another name, and the circle in §8 would be closed at the first step. The whole point of naming an object that is not a Hilbert-space world is that identity of objects is not a Hilbert-space question. The identity criterion does not wait on a later grain.
...
The identity criterion of §4 is a premise: two evolutions split at the first $\tau$ the simply-connected analytic patches disagree. The infinite jet, analyticity, and the unique simply-connected patch are locked. Extra topologies are the relative ensemble, not a second elementary observer. This paper does not settle the other open items. The dimension of observer space is open. The mechanism of non-uniqueness is open, and is not identified with closed timelike curves. Matter fields are open. Gauge is the jet/patch pair as one lump. Those are not hidden assumptions. They are absences.

The 2008 costume is left on the shelf. This paper does not revive $\chi$, 4-geons, spin foam, Wheeler's geons-as-particles picture, or Hadley's CTCs-as-particles picture as current claims. It does not say that all is geometry. It does use unique analytic continuation to determine the simply-connected local patch from the infinite jet. It does not use analytic continuation to reconstruct a whole four-manifold $W$ from an observer $O$. It does not make the observer a 4-ball that determines a whole spacetime. The 2008 time direction, and the 2008 infinite jet as a local object, are retained. The 2008 reconstruction of a global $W$ is not.
