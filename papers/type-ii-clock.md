...
**Status.** Theorem 23(1): hitchhiker $\tau$ does not label type-(ii) targets (packaging of Theorem 22(1)). (2a): an extra $W$ from $E(O)$ is $E$-smuggling. (2b): $W=P(O)$ is locked; Lorentzian distance $\tau(p,q)$ (supremum of proper times of causal curves $p\to q$; length of a maximizing geodesic in a convex neighbourhood) is lock-side. Given a fixed $q$, $\tau(p,q)$ is unique when defined. Leftover non-uniqueness in $(O,O')$ is only multiple occurrence of $O'$ in $P(O)$. Infimum among causal curves is typically $0$ (near-null) and is not a clock. Not a general type-(ii) clock. Not a closed “no real function of $(O,O')$.” Leftover candidates (generation, Lyapunov, walk-length) are extra, not adopted.
...
2. Split:
   ...
   - *(2b) $W=P(O)$.* The patch is locked data, not an extra topology. If $O'$ occurs at some $q\in P(O)$ with $q\notin\sigma_O$, and if $(O,O')$ is edged, that germ is type (ii). The lock-side quantity is the Lorentzian distance (time-separation)
$$
\tau(p,q)\;=\;\sup\{\,L(\gamma):\ \gamma\text{ causal in }P(O)\text{ from }p\text{ to }q\,\}\in[0,\infty],
$$
equivalently the length of a maximizing geodesic in a convex neighbourhood of $p$. Given a *fixed* $q$, this $\tau(p,q)$ is unique when defined. The infimum of proper times among causal curves $p\to q$ is typically $0$ (near-null curves) and is not a clock. Non-uniqueness in the pair of lumps $(O,O')$ remains only if $O'$ occurs at more than one $q$ in $P(O)$. If $O'$ does not occur in $P(O)$, $\tau(p,\cdot)$ is undefined for that pair. Most type-(ii) pairs are of that kind, so this is not a general type-(ii) clock.
...
**Proof of (2b).** $P(O)$ is the unique simply-connected analytic continuation of the germ (Theorem B). It is not an ensemble extra. A point $q\in P(O)\setminus\sigma_O$ determines a lump $O_q$ off $\gamma_O$. An edge $(O,O_q)$ is type (ii) if it lies in $R$. Causal curves from $p$ to $q$ in $P(O)$, when they exist, have proper times; their *supremum* $\tau(p,q)$ is the Lorentzian distance, read off the locked metric. In a convex (normal) neighbourhood this is the length of the unique maximizing geodesic. The sup, when the set of causal curves is nonempty, is a single extended-real number, so $\Delta(p,q)$ is unique given that $q$. Globally the sup may fail to be attained, or be $+\infty$. The infimum is typically $0$ and is discarded as a clock. If $O'$ occurs at $q_1$ and $q_2$, $\tau(p,q_1)$ and $\tau(p,q_2)$ need not agree, so the construction does not yield a unique $\Delta\tau(O,O')$. If $O'$ does not occur in $P(O)$, the construction is undefined for that pair. The set of lumps that occur in $P(O)$ is the hitchhiking-plus-spatial content of one patch, not all of $\mathrm{Obs}$. $\square$

**Corollary 23.1.** Proper-time-in-some-extra-$W$ is $E$-smuggling (2a). In-patch Lorentzian distance $\tau(p,q)$ for a *fixed* $q\in P(O)\setminus\sigma_O$ is lock-side, unique when defined, and is *not* type-(i) hitchhiking (the maximizing curve need not be $\sigma_O$). Infimum among causal curves is not a clock. Non-uniqueness in $(O,O')$ is only multiple occurrence of $O'$ in $P(O)$. The construction is undefined for type-(ii) pairs that do not occur in $P(O)$.

**Not claimed.** There is no real function of $(O,O')$. The constant $0$ is a function. Walk-length along $R$ is a function of a law of $R$, which is extra (Theorem 19). Locally finite one-step out-neighborhoods are finite slices from that law (Theorem 22). Every proper-time-in-$W$ is type (i) or $E$-smuggling: that overclaim is not restored.
...
| In-patch Lorentzian distance $\tau(p,q)$ | Sup of proper times of causal curves in $P(O)$ to a fixed $q$ off $\sigma_O$ (maximizing geodesic in a convex neighbourhood) | Yes, unique given $q$ when defined; not unique in $(O,O')$ if several $q$; not general | no |
...
- **Proved.** Theorem 23(1): hitchhiker $\tau$ does not timestamp type (ii). (2a): extra $W$ from $E(O)$ is $E$-smuggling. (2b): in-patch Lorentzian distance (sup) is lock-side and unique given a fixed $q$ when defined; leftover non-uniqueness in $(O,O')$ is only multiple occurrence; not a general type-(ii) clock. Infimum among causal curves is typically $0$, not a clock.
- **Not claimed.** No real function of $(O,O')$. Every proper-time-in-$W$ is type (i) or $E$-smuggling.
- **Named, not adopted.** Generation, Lyapunov, walk-length. In-patch Lorentzian distance not adopted as a general clock.
...
Hawking, S. W. and Ellis, G. F. R. *The Large Scale Structure of Space-Time.* Cambridge, 1973. (Time-separation as a supremum.)
