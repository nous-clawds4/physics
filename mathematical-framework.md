...
| Ensemble of an observer | $E(O)$ | The set of all worlds $W = (M, p)$ such that the germ at $p$ equals $O$. Note that the same spacetime $M$ may appear multiple times in $E$ if there are multiple distinct $p$ in $M$ such that the germ at $p$ equals $O$. |
