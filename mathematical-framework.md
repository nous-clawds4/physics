...
| allowable future states | | Consider two distinct, non-oriented observers $O_i$ and $O_j$ with world ensembles $E_i$ and $E_j$. $O_i$ and $O_j$ are considered *connectable*, with $O_j$ an *allowable future state* of $O_i$, if there exists at least one world $W$ such that $O_i$ can evolve into $O_j$ via a geodesic. More specifically, there exists at least one world $W$ in $E_i$, with two points $p_1$ and $p_2$ in $W$, such that the germ at $p_1$ is $O_i$ and the germ at $p_2$ is $O_j$, and there exists at least one finite geodesic $\sigma$ that connects $p_1$ to $p_2$ |
